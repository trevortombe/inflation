...
# CPI product list
product_list<-read.csv("ChenTombeReplication/Files/cpi_products.csv")
...
write.csv(plotdata %>%
            spread(product,contrib),"ChenTombeReplication/Figures/Figure1_data.csv",row.names = F)
...
write.csv(results %>% select(Date=Ref_Date,contrib_annual,
            ...
            mutate(Date=as.yearqtr(Date)),"ChenTombeReplication/Files/Results_2010_Latest_Annual.csv",row.names = F)
